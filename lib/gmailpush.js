...
Gmailpush.prototype = {
  ...
  async _initialize(notification, token) {
    ...
    try {
      ...
      if (err.message.startsWith('ENOENT: no such file or directory')) {
        this._api.prevHistories.push({
          ...
          watchExpiration: await this._refreshWatch(),
        });
      } else {
        throw err;
      }
    ...
    // Call watch() one day before expiration
    if (Date.now() >= prevHistory.watchExpiration - 24 * 60 * 60 * 1000) {
  ...
  _parseMessage(message, historyEntry) {
    ...
    if (from) {
      parsedMessage.from = {
        name:
          from.value.match(emailField)[1] || from.value.match(emailField)[2],
        address: from.value.match(emailField)[2],
      };
    }
    ...
    if (to) {
      parsedMessage.to = [];
      to.value.split(', ').forEach((e) => {
        parsedMessage.to.push({
          name: e.match(emailField)[1] || e.match(emailField)[2],
          address: e.match(emailField)[2],
        });
      });
    }
    ...
    if (cc) {
      parsedMessage.cc = [];
      cc.value.split(', ').forEach((e) => {
        parsedMessage.cc.push({
          name: e.match(emailField)[1] || e.match(emailField)[2],
          address: e.match(emailField)[2],
        });
      });
    ...
    if (bcc) {
      parsedMessage.bcc = [];
      bcc.value.split(', ').forEach((e) => {
        parsedMessage.bcc.push({
          name: e.match(emailField)[1] || e.match(emailField)[2],
          address: e.match(emailField)[2],
        });
      });
